...
javax.swing используется для графиичесского представления программы 
...
В методе main инициализируем объект класса и вызоваем метод go:
```java
...
В методе go создаем фрейм и добавляем в него разные элементы, а так же задаем им параметры и создаем возможность для взаимодействия с ними.
Разберем немного подробнее:

Для начала нужно инициализировать фрейм и задать ему имя, создать панель, на которую мы будем размещать элементы, а так же создать поле, на которое будет выводиться информация и сразу добавить ее на панедль:
```java
	public void go() {
...
Далее необхожимо проделать то же самое с кнопками цифр:
```java
		b1 = new JButton("1");
...
И наконец настраиваем раброту фрейма:
```java
...
Сначала необходимо указать методу праметр типа ActionEvent, так же создать переменную для хранения значения с поля для вывода информации:
```java
